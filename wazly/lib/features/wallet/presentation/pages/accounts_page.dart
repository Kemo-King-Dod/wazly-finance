...
class _AccountsPageState extends State<AccountsPage> {
  ...
  void initState() {
    super.initState();
    // Fetch wallet data when page loads (includes accounts)
    ...
    return Scaffold(
      ...
      drawer: BlocBuilder<WalletBloc, WalletState>(
        builder: (context, state) {
          ...
          } else if (state is WalletAccountsLoaded) {
            debtAssets = state.debtAssets;
            debtLiabilities = state.debtLiabilities;
          }
              ...
              Padding(
                ...
                child: TextField(
                  onChanged: (query) {
                    ...
                    final currentFilter = currentState is WalletAccountsLoaded
                        ? currentState.filter
                        : AccountFilter.all;
                    context.read<WalletBloc>().add(
                      SearchAccounts(query: query, filter: currentFilter),
                    );
                  },
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: AccountFilter.values.map((filter) {
                    final isSelected =
                        (context.watch<WalletBloc>().state
                            is WalletAccountsLoaded)
                        ? (context.watch<WalletBloc>().state
                                      as WalletAccountsLoaded)
                                  .filter ==
                              filter
                        ...
                        onSelected: (selected) {
                          if (selected) {
                            final currentState = context
                                .read<WalletBloc>()
                                .state;
                            final query = currentState is WalletAccountsLoaded
                                ? currentState.searchQuery
                                : '';
                            context.read<WalletBloc>().add(
                              SearchAccounts(query: query, filter: filter),
                            );
                          }
                        },
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              context.read<WalletBloc>().add(DeleteAccountEvent(account.id));
              Navigator.pop(context);
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: AppTheme.debtColor,
