...
import '../../../accounts/presentation/blocs/account_event.dart';
  ...
  void _handleSave() {
    if (_formKey.currentState!.validate()) {
      ...
      if (_debtType == DebtType.theyOweMe) {
        isIncome = true; // Income: money I will receive (they owe me)
      } else if (_debtType == DebtType.iOweThem) {
        isIncome = false; // Expense: money I will pay (I owe them)
      } else {
      ...
      if (_debtType == DebtType.theyOweMe || _debtType == DebtType.iOweThem) {
        context.read<DebtBloc>().add(AddDebt(transaction));
      } else {
        context.read<DebtBloc>().add(AddSettlement(transaction));
      }
    }
  }
    ...
    return BlocListener<DebtBloc, DebtState>(
      listener: (context, state) {
        if (state is DebtSuccess) {
          ...
          // Refresh transactions and accounts after adding debt
      ...
      child: Scaffold(
        ...
        body: BlocBuilder<AccountBloc, AccountState>(
          builder: (context, state) {
            final isLoadingAccount = state is AccountAccountsLoading;

            // Load accounts from state
            if (state is AccountAccountsLoaded && _allAccounts.isEmpty) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted) {
                  setState(() {
            ...
            return BlocBuilder<DebtBloc, DebtState>(
              builder: (context, debtState) {
                final isSaving = debtState is DebtLoading;
                ...
                return Form(
                  ...
                  child: ListView(
                    ...
                    children: [
                      // Account Selection
                      if (isLoadingAccount && _allAccounts.isEmpty)
                      ...
                      // Save Button
                      _buildSaveButton(l10n, isSaving),
                    ],
                  ),
                );
              },
